        ...
        if player_choice in self.options:
            ...
            if player_choice == bot_choice:
                await self.ctx.channel.send(
                    'Ni pa ti ni pa mi, empate.:exploding_head: ' + first_to_uppercase(
                        player_choice) + ' shake hands ' + first_to_uppercase(bot_choice))
            for i in self.win_conditions:
                if bot_choice == i[0] and player_choice == i[2]:
                    await self.ctx.channel.send(
                        'Lo siento pero... He ganado:sunglasses: ' + first_to_uppercase(i[0]) + ' ' + i[
                            1] + ' ' + first_to_uppercase(i[2]))
                if player_choice == i[0] and bot_choice == i[2]:
                    await self.ctx.channel.send(
                        'Tu ganas :tired_face: ' + first_to_uppercase(i[0]) + ' ' + i[1] + ' ' + first_to_uppercase(
                            i[2]))
        else:
            await self.ctx.channel.send("Tienes que poner una de estas opciones: Rock, Paper, Scissor, Lizard, Spock.")
